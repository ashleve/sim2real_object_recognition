...
void initPID()
{
  motorRight.SetOutputLimits(MIN_POWER, MAX_POWER);  
  motorLeft.SetOutputLimits(MIN_POWER, MAX_POWER);  
  motorRight.SetMode(AUTOMATIC);  
  motorLeft.SetMode(AUTOMATIC);  
  motorRight.SetSampleTime(CALCULATION_PERIOD);  
  motorLeft.SetSampleTime(CALCULATION_PERIOD);  
}
